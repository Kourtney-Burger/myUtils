...
# for testing
raw_dir <- "C:/Users/kourtney.burger/Documents/Gliders/dmon/risso_April_deployment/test/raw/"
processed_dir <- "C:/Users/kourtney.burger/Documents/Gliders/dmon/risso_April_deployment/test/processed/"
...
# ----Process wavs----
# ----Process wavs----
## ----Copy and Rename .wav2 and .wav3 files using log start time----

# Set output directory
renamed_wav_dir <- file.path(processed_dir, "renamed_wav_startTime")
...
for (file in wav_list) {
  ...
  # Determine hydrophone type and expected wav suffix
  if (grepl("\\.wav2$", base, ignore.case = TRUE)) {
    suffix <- "_LF.wav"
    log_match_name <- sub("\\.wav2$", "_LF.wav", base, ignore.case = TRUE)
  } else if (grepl("\\.wav3$", base, ignore.case = TRUE)) {
    suffix <- "_HF.wav"
    log_match_name <- sub("\\.wav3$", "_HF.wav", base, ignore.case = TRUE)
  } else {
    next  # Skip if neither
  }
  
  # Filter logs matching this wav file
  matched_logs <- final_df %>%
    filter(source_wav == log_match_name)
  
  if (nrow(matched_logs) == 0) {
    warning(paste("No logs found for:", base))
    next
  }
  
  # Get the earliest start time
  first_time <- min(as.POSIXct(matched_logs$start_time, tz = "UTC"))
  
  # Format the timestamp for filename
  formatted_time <- format(first_time, "%Y%m%d_%H%M%S")
  
  # Extract original base name without extension (remove .wav2/.wav3)
  original_base <- tools::file_path_sans_ext(base)
  
  # Build new name using original base name + timestamp + suffix
  new_name <- paste0(original_base, "_", formatted_time, suffix)
...
# TRYING TO APPLY CHANGES BUT NOT WORKING FOR ALL SCENARIOS 
for (i in 1:nrow(LF_logs)) {
# for testing
# for (i in 1:28) {
    # create name string and path for segmented wav
